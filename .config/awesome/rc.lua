...
xkbSwapEscape = 'setxkbmap -option caps:swapescape'
xkbDvorak = 'setxkbmap -layout us -variant dvorak'
xkbUa = 'setxkbmap ua'

function buildScreenLockCommand(mode)
  return 'xlock -fontset "-misc-dejavu serif condensed-medium-i-semicondensed--0-0-0-0-p-0-ascii-0" -mode '
    .. mode
end
...
mydocs = {
  ...
  { 'Calc', 'libreoffice --calc', newaita_path .. 'libreoffice-calc.svg' },
  {
    'Writer',
    'libreoffice --writer',
    newaita_path .. 'libreoffice-writer.svg',
  },
}
...
awful.widget
  .keyboardlayout()
  :connect_signal('button::press', function(_, _, _, button)
    if button == 1 then -- left click
      awful.spawn.with_shell(xkbDvorak .. ' && ' .. xkbSwapEscape)
    elseif button == 3 then -- right click
      awful.spawn.with_shell(xkbUa .. ' && ' .. xkbSwapEscape)
    end
  end)
...
globalkeys = gears.table.join(
  ...
  awful.key(
    { modkey, 'Control' },
    'l',
    function() awful.spawn.with_shell(buildScreenLockCommand 'fiberlamp') end,
...
clientkeys = gears.table.join(
  ...
  awful.key(
    { 'Control' },
    '7',
    function() awful.spawn.with_shell(xkbDvorak .. ' && ' .. xkbSwapEscape) end,
  ...
  awful.key(
    ...
    '5',
    function()
      awful.spawn.with_shell('setxkbmap -layout us && ' .. xkbSwapEscape)
    end,
  ...
  awful.key(
    { 'Control' },
    '8',
    function() awful.spawn.with_shell(xkbUa .. ' && ' .. xkbSwapEscape) end,
    { description = 'switch keyboard layout to uk', group = 'client' }
  ),
  awful.key(
    { 'Control' },
    '9',
    function() awful.spawn.with_shell('setxkbmap ru && ' .. xkbSwapEscape) end,
...
awful.rules.rules = {
  -- All clients will match this rule.
  {
    ...
    properties = {
      ...
      shape = function()
        return function(cr, w, h) gears.shape.rounded_rect(cr, w, h, 8) end
      end,
    },
  },
